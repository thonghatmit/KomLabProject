...
//FRIDOLIN, to access server object in cli.c file
struct pppoe_serv_t *global_serv;
...
{
	printf("Executing allocate_channel()\n"); //FRIDOLIN
	...
	printf("at allocate_channel sid generated = %u\n", conn->sid); //FRIDOLIN
	pthread_mutex_unlock(&sid_lock);
...
void test_in_pppoe(uint8_t test_int){
	printf("test_in_pppoe() called with %i\n", test_int);
}

//FRIDOLIN: modified version of allocate_channel()
struct pppoe_conn_t *allocate_channel_migrated(struct pppoe_serv_t *serv, const uint8_t *addr, const uint8_t *cookie, const uint16_t migrated_sid)
{
	struct pppoe_conn_t *conn;
	unsigned long *old_sid_ptr;
	...
	pthread_mutex_lock(&sid_lock);
	conn->sid = migrated_sid;
	printf("at allocate_channel migrated sid MIRATED = %u\n", conn->sid);
	// old_sid_ptr = sid_ptr;
	// while (1) {
	// 	int bit = ffsl(*sid_ptr) - 1;
	...
	// 		break;
	// }
	// printf("at allocate_channel migrated sid generated = %u\n", conn->sid); //FRIDOLIN
	pthread_mutex_unlock(&sid_lock);
	...
	struct pppoe_tag *service_name = _malloc(sizeof(service_name)); //FRIDOLIN: moved here from passed arguments as tag_len is always 0
	service_name->tag_len = 0;
	...
	conn->ctx.before_switch = pppoe_conn_ctx_switch;
	conn->ctx.close = pppoe_conn_close; //FRIDOLIN TODO: prevent closing of fresh created session (migrated) - or terminate?
	conn->ctrl.ctx = &conn->ctx;
	conn->ctrl.started = ppp_started;
	conn->ctrl.finished = ppp_finished;
	conn->ctrl.terminate = ppp_terminate;
	...
	//Added
	triton_context_call(&conn->ctx, (triton_event_func)connect_channel_non_static, conn); //FRIDOLIN: connect channel creates ppp interfaces
...
//FRIDOLIN
void connect_channel_non_static(struct pppoe_conn_t *conn)
{
	int sock;
	struct sockaddr_pppox sp;
	printf("connect_channel_non_static() called\n"); //FRIDOLIN
	...
	if (establish_ppp_migration(&conn->ppp)) //FRIDOLIN: modified establish_ppp() without LCP layer
		goto out_err_close;
...
out_err_close:
	printf("out_err_close!\n");
	close(sock);
out_err:
	disconnect(conn);
}
//FRIDOLIN: only comment: function to find already allocated channel
...
static void pppoe_recv_PADR(struct pppoe_serv_t *serv, uint8_t *pack, int size)
{
	...
	pthread_mutex_lock(&serv->lock);
	// FRIDOLIN: conn = pppoe_conn_t which contains ppp struct which contains ap_session struct
	...
	if (!conn)
		...
		pppoe_send_PADS(conn);
		triton_context_call(&conn->ctx, (triton_event_func)connect_channel, conn); //FRIDOLIN: connect channel creates ppp interfaces
		triton_context_wakeup(&conn->ctx);
	}
}
...
{
	//FRIDOLIN: serv->conn_list = {next = 0x5555557c6700, prev = 0x5555557c6700} (=correct, why is container_of working here with right addr but not in cli.c?)
	struct pppoe_serv_t *serv = container_of(triton_context_self(), typeof(*serv), ctx);

	//FRIDOLIN: backup address of pppoe_serv_t struct to acess in cli.c
	global_serv = serv;
