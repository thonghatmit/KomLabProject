...
exit_close_chan:
	printf("exit_close_chan at ppp.c\n");
	close(ppp->chan_fd);

	return -1;
}

//FRIDOLIN: modified establish_ppp() func without LCP layer as configuration is already done
int __export establish_ppp_migration(struct ppp_t *ppp)
{
	printf("establish_ppp()\n");
	if (ap_shutdown)
		return -1;
...
cont:
		...
		if(ppp->ses.is_migrated != 1) //FRIDOLIN
			lcp_send_proto_rej(ppp, proto);
		else
			printf("Ignore calling lcp_send_proto_rej() for migrated session\n"); //FRIDOLIN
...
static void init_layers(struct ppp_t *ppp)
{
	...
	list_for_each_entry(n,&layers,entry) {
		...
		list_for_each_entry(l, &n->items, entry) {
			d = l->init(ppp); // FRIDOLIN comment: calls init() function of e.g. lcp layer
			d->layer = l;
			d->started = 0;
			d->node = n1;
